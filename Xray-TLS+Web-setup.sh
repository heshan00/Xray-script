...
check_script_update()
{
    [ "$(md5sum "${BASH_SOURCE[0]}" | awk '{print $1}')" == "$(md5sum <(wget -O - "https://github.com/heshan00/Xray-script/raw/main/Xray-TLS+Web-setup.sh") | awk '{print $1}')" ] && return 1 || return 0
}
#更新脚本
update_script()
{
    if wget -O "${BASH_SOURCE[0]}" "https://github.com/heshan00/Xray-script/raw/main/Xray-TLS+Web-setup.sh" || wget -O "${BASH_SOURCE[0]}" "https://github.com/heshan00/Xray-script/raw/main/Xray-TLS+Web-setup.sh"; then
    ...
    if [ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]; then
        if LANG="en_US.UTF-8" LANGUAGE="en_US:en" dpkg -s "$1" 2>/dev/null | grep -qi 'status[ '$'\t]*:[ '$'\t]*install[ '$'\t]*ok[ '$'\t]*installed[ '$'\t]*$'; then
            if LANG="en_US.UTF-8" LANGUAGE="en_US:en" apt-mark manual "$1" | grep -qi 'set[ '$'\t]*to[ '$'\t]*manually[ '$'\t]*installed'; then
                ...
                red "安装依赖 \"$1\" 出错！"
                green  "欢迎进行Bug report(https://github.com/heshan00/Xray-script/issues)，感谢您的支持"
...
swap_off()
{
    if [ $using_swap_now -eq 1 ]; then
        ...
        if swapoff ${temp_dir}/swap && rm -rf ${temp_dir}/swap; then
            ...
            red    "关闭swap失败！"
            green  "欢迎进行Bug report(https://github.com/heshan00/Xray-script/issues)，感谢您的
支持"
            yellow "按回车键继续或者Ctrl+c退出"
            read -s
        fi
        ...
        if (( 1<=choice&&choice<=4 )); then
            if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]) && ! dpkg-deb --help | grep -qw "zstd"; then
                ...
                if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]) && ! version_ge "$(dpkg --list | grep '^[ '$'\t]*ii[ '$'\t][ '$'\t]*linux-base[ '$'\t]' | awk '{print $3}')" "4.5ubuntu1~16.04.1"; then
                    ...
                    if [ $choice -eq 1 ]; then
                        temp_kernel_sh_url="https://github.com/heshan00/update-kernel/raw/master/update-kernel-stable.sh"
                    elif [ $choice -eq 4 ]; then
                        temp_kernel_sh_url="https://github.com/heshan00/update-kernel/raw/master/update-kernel.sh"
                    else
                        temp_kernel_sh_url="https://github.com/heshan00/xanmod-install/raw/main/xanmod-install.sh"
                    fi
...
readProtocolConfig()
{
    ...
    blue   "   2. gRPC和WebSocket支持通过CDN，关于两者的区别，详见：https://github.com/heshan00/Xray-script#关于grpc与websocket"
    blue   "   3. 仅TCP能使用XTLS"
    echo
    local choice=""
...
readPretend()
{
    ...
    do
        ...
        echo
        yellow " 关于选择伪装网站的详细说明见：https://github.com/heshan00/Xray-script#伪装网站说明"
        echo
        pretend=""
...
print_config_info()
{
    ...
    echo
    yellow " 关于fingerprint与alpn，详见：https://github.com/heshan00/Xray-script#关于tls握手tls指纹和alpn"
    echo
...
start_menu()
{
    ...
    echo
    tyblue "       官网：https://github.com/heshan00/Xray-script"
    echo
    tyblue "----------------------------------注意事项----------------------------------"
    yellow " 1. 此脚本需要一个解析到本服务器的域名"
    tyblue " 2. 此脚本安装时间较长，建议在安装前阅读："
    tyblue "      https://github.com/heshan00/Xray-script#安装时长说明"
